...
try:
    ...
    print(f"⚠️ Field detection not available: {e}")
...
if FIELD_DETECTION_AVAILABLE:
    try:
        ...
        print("✅ Field detection system initialized")
    except Exception as e:
        print(f"⚠️ Field detection initialization failed: {e}")
        FIELD_DETECTION_AVAILABLE = False
...
def document_processing():
    """Document processing page for Word, PDF, and XML files."""
    return render_template('document_processing.html')

@app.route('/field-detection')
@app.route('/field_detection')
def field_detection():
    """Field detection page for advanced field extraction from documents."""
    ...
    try:
        ...
        try:
            ...
            if doc_type == 'mrz':
                ...
                print(f"🔧 Using OCR engine: {ocr_engine} (Fast mode: {fast_mode})")
                
                # Use fast mode if requested
                if fast_mode:
                    print("⚡ Using fast OCR mode for quick processing")
...
def process_complete_ocr_flow():
    ...
    try:
        ...
        try:
            logger.info(f"🔄 Starting complete OCR flow for: {filename}")
            ...
            if not flow_result.get('success', False):
                ...
                logger.error(f"❌ Complete OCR flow failed at {failed_step}: {error_msg}")
                return jsonify({
                    'success': False,
            ...
            logger.info(f"✅ Complete OCR flow successful: {flow_result['flow_id']}")
            logger.info(f"📊 Summary: {processing_summary.get('fields_detected', 0)} detected, {processing_summary.get('fields_blurred', 0)} blurred")
        ...
        logger.error(f"❌ Complete OCR flow endpoint error: {e}")
        return jsonify({
            'success': False,
